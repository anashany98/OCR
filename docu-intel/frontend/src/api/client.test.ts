...
describe("new operations API client methods", () => {
  ...
  it("posts work inbox actions", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse({ action: "retry_failed_jobs", matched: 1, updated: 0, enqueued: 1, job_ids: [9] }))
  ...
  it("posts redaction preview requests", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse({ redacted_text: "Total [IMPORTE OCULTO]" }))

    await api.redactionPreview({ principal_type: "technician", principal_id: "tech-1", text: "Total 99,00 €" })
  ...
  it("loads production readiness checks", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse({ status: "ready", checks: [] }))
  ...
  it("loads data quality summary", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse({ rules: {}, by_quality_status: {} }))
  ...
  it("posts bulk document tags", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse({ matched: 1, updated: 1 }))
    ...
    expect(JSON.parse(String(fetchMock.mock.calls[0][1]?.body))).toEqual({ document_ids: [7], add_tags: ["contabilidad"], remove_tags: [] })
  })

  it("creates persisted work item comments", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse({ id: 4, body: "Revisado" }))
  ...
  it("posts OCR revisions for document pages", async () => {
    ...
    await api.createOcrRevision(12, { corrected_text: "Texto corregido", reason: "Corrección manual" })
  ...
  it("saves document search presets", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse({ id: 8, name: "Facturas" }))

    await api.createSavedSearch({ name: "Facturas", query: "factura", mode: "hybrid", filters_json: { document_type: "invoice" } })
  ...
  it("generates and updates reconciliation issues", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(jsonResponse([])).mockResolvedValueOnce(jsonResponse({ id: 5, status: "reviewed" }))

    await api.generateReconciliationIssues()
    await api.updateReconciliationIssue(5, { status: "reviewed", resolution_notes: "Validado por compras" })
  ...
  it("creates invoices for reconciliation", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse({ id: 7, invoice_number: "F-1" }))
  ...
  it("records manual plan measurements", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(jsonResponse({ id: 6, has_discrepancy: true }))

    await api.createPlanMeasurement(2, { label: "Largo pasillo", value_m: 3.4, ocr_value_m: 3.1, points_json: [] })
