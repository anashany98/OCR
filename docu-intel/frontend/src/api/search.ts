...
export const searchApi = {
  exportSearchCSV: (query: string, limit = 100) => {
    window.open(
      `${API_BASE_URL}` + "/search/export/csv?q=" + encodeURIComponent(query) + "&limit=" + limit,
      "_blank",
    )
  },
  exportSearchJSON: (query: string, limit = 100) => {
    window.open(
      `${API_BASE_URL}` + "/search/export/json?q=" + encodeURIComponent(query) + "&limit=" + limit,
      "_blank",
    )
  },
  textSearch: (query: string) =>
    request<SearchResult[]>(`/search/text?q=` + encodeURIComponent(query)),
  guidedSearch: (query: string, mode: string) =>
    request<SearchResult[]>(
      `/search/guided?mode=` + encodeURIComponent(mode) + `&q=` + encodeURIComponent(query),
    ),
