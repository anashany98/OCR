...
export const adminApi = {
  ...
  runWorkInboxAction: (payload: { action: string; limit?: number; min_confidence?: number }) =>
    request<WorkInboxActionResponse>("/admin/work-inbox/actions", {
      method: "POST",
  ...
  updateWorkItem: (
    ...
    request<WorkItem>("/admin/work-items/" + id, {
      method: "PATCH",
      body: JSON.stringify(payload),
    }),
  addWorkItemComment: (id: number, payload: { body: string }) =>
    request<WorkItemComment>("/admin/work-items/" + id + "/comments", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  savedViews: (scope = "documents") =>
    request<SavedView[]>("/admin/saved-views" + buildSearchParams({ scope })),
  ...
  createNotificationRule: (payload: {
    ...
    request<NotificationRule>("/admin/notification-rules", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  ...
  rulePreview: (payload: {
    path: string
    pattern: string
    match_type?: "contains" | "glob" | "regex"
    tags_json?: string[]
  }) =>
    request<RulePreview>("/admin/rules/preview", { method: "POST", body: JSON.stringify(payload) }),
  redactionPreview: (payload: {
    principal_type: "user" | "technician"
    principal_id: string
    text: string
  }) =>
    request<RedactionPreview>("/admin/security/redaction-preview", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  ...
  storageIntegrity: (limit = 1000) =>
    request<StorageIntegrity>("/admin/storage/integrity?limit=" + limit),
  ...
  recalculateQuality: (payload: { limit?: number }) =>
    request<QualityRecalculateResponse>("/admin/quality/recalculate", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  ...
  bulkDocumentTags: (payload: {
    document_ids: number[]
    add_tags?: string[]
    remove_tags?: string[]
  }) =>
    request<BulkTagsResponse>("/admin/documents/bulk-tags", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  ...
  accessExplain: (params: {
    principal_type: "user" | "technician"
    principal_id: string
    document_id: number
  }) => request<AccessExplain>("/admin/access-explain" + buildSearchParams(params)),
  documentGraph: (documentId: number) =>
    request<DocumentGraph>("/admin/documents/" + documentId + "/graph"),
  ...
  updateOcrReview: (
    pageId: number,
    payload: { review_status: "pending" | "approved" | "rejected"; review_notes?: string | null },
  ) =>
    request<OcrReviewPage>("/admin/ocr-review/" + pageId, {
      method: "PATCH",
      body: JSON.stringify(payload),
    }),
  reprocessOcrPage: (pageId: number) =>
    request<Job>("/admin/quality/pages/" + pageId + "/reprocess-ocr", { method: "POST" }),
  ...
  applyFolderRules: (force = false) =>
    request<{ matched: number; assigned: number; quarantined: number; skipped: number }>(
      "/admin/folder-rules/apply",
      {
        method: "POST",
  ...
  upsertAccessGroupMember: (
    groupId: number,
    payload: { principal_type: "user" | "technician"; principal_id: string },
  ) =>
    request<AccessGroupMember>(`/admin/access-groups/` + groupId + `/members`, {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  sensitiveTags: () => request<SensitiveTag[]>(`/admin/sensitive-tags`),
  createSensitiveTag: (payload: {
    name: string
    description?: string | null
    is_active?: boolean
  }) =>
    request<SensitiveTag>("/admin/sensitive-tags", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
}
