        ...
        <div className="flex min-w-0 flex-1 flex-col lg:flex-[1]">
          ...
          >
            <TabsList className="w-full justify-start rounded-lg bg-[var(--bg-surface-2)] px-1">
              ...
              <TabsTrigger value="ocr" className="gap-1.5 text-[12px]">
                <Type className="h-3 w-3" /> OCR
              </TabsTrigger>
              <TabsTrigger value="entities" className="gap-1.5 text-[12px]">
                <ShieldAlert className="h-3 w-3" /> Entidades
              </TabsTrigger>
              <TabsTrigger value="timeline" className="gap-1.5 text-[12px]">
                <Network className="h-3 w-3" /> Timeline
              </TabsTrigger>
            </TabsList>
    ...
    <div className="flex items-center gap-3 rounded-lg border border-[var(--border)] bg-[var(--bg-surface)] px-4 py-2.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex flex-wrap items-center gap-1.5">
          ...
          {doc?.quality_status && doc.quality_status !== "processed_ok" && (
            <StatusBadge status={doc.quality_status} />
          )}
          <ConfidenceBadge value={doc?.confidence} />
          {doc?.error_message && (
            <span
              className="inline-flex items-center gap-0.5 rounded bg-[var(--danger-light)] px-1.5 py-0.5 text-[10px] text-[var(--text-on-danger)]"
  ...
  return (
    <div className="flex flex-wrap gap-1">
        ...
        <PermissionGate roles={["admin", "gestor"]}>
          <Button asChild variant="outline" size="sm" className="h-7 text-[11px]">
            <Link to={`/documents/${id}/annotate-plan`}>
              <MapPin className="mr-1 h-3 w-3" /> Anotar
            </Link>
          </Button>
        </PermissionGate>
      )}
      <PermissionGate roles={["admin"]}>
        ...
        >
          <RefreshCcw className="mr-1 h-3 w-3" /> Reprocesar
        </Button>
      </PermissionGate>
      {id && (
...
function ViewerCard({ d }: { d: ReturnType<typeof useDocumentDetail> }) {
  ...
  const excelText =
    isExcel && d.pages.length > 0 ? d.pages.map((p) => p.text || "").join("\n\n") : ""
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col p-0">
        <div className="flex min-h-0 flex-1 flex-col">
          {isExcel && excelText ? (
            ...
            <FallbackPreview
              doc={doc}
              message="No se pudo generar la imagen de esta página. El PDF puede estar dañado o el procesamiento no ha terminado."
            />
          ...
          <div className="flex flex-wrap gap-1 border-t border-[var(--border)] bg-[var(--bg-surface-2)] px-3 py-1.5">
            {d.pages.map((p) => (
              <Button
                ...
                type="button"
                size="sm"
                variant={p.page_number === page?.page_number ? "default" : "ghost"}
                className="h-6 px-2 text-[10px]"
                onClick={() => {
                  d.setSelectedPageNumber(p.page_number)
                  setImgError(false)
                }}
              >
                P{p.page_number}
...
function FallbackPreview({
  ...
  doc: { original_filename: string; extension?: string | null; file_size: number }
  message: string
}) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-3 bg-[var(--bg-surface-2)] p-6 text-center">
      ...
      <p className="text-[10px] text-[var(--text-muted)]">
        {doc.extension} · {formatBytes(doc.file_size)}
      </p>
    </div>
  )
}
  ...
  return (
    <div className="space-y-3 p-1">
      <Card>
        <CardContent className="space-y-2 p-3 text-[12px]">
          <div className="grid grid-cols-2 gap-2">
            ...
            <InfoRow
              label="Tamaño"
              value={doc?.file_size != null ? formatBytes(doc.file_size) : "—"}
            />
      ...
      {d.keyEnts.length > 0 && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-[13px]">Entidades clave</CardTitle>
          </CardHeader>
      ...
      {d.otherEnts.length > 0 && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-[13px]">Otras entidades ({d.otherEnts.length})</CardTitle>
          </CardHeader>
...
function InfoRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex justify-between gap-2">
      ...
      <span
        className={cn("text-right text-[var(--text-primary)]", mono && "font-mono text-[10px]")}
      >
        {value}
      </span>
    </div>
      ...
      <div className="space-y-2">
        {d.visiblePages.map((p) => (
          <section key={p.id} className="rounded-md border bg-[var(--bg-surface)] p-2.5">
            ...
            <pre className="whitespace-pre-wrap font-sans text-[12px] leading-5">
              <HighlightedText text={page?.text || "Sin texto extraído."} query={d.textQuery} />
            </pre>
          </section>
        ))}
        {!d.visiblePages.length && (
          <EmptyState title="Sin coincidencias" description="No hay páginas que coincidan." />
        )}
      </div>
      {page && (
        <div className="rounded-md border bg-[var(--bg-surface-2)] p-2.5">
          <div className="mb-1.5 flex items-center justify-between">
            ...
            <Button
              ...
              disabled={
                d.saveRevision.isPending ||
                !d.editedText.trim() ||
                d.editedText === (page.text ?? "")
              }
            >
          ...
          <textarea
            className="min-h-[80px] w-full rounded border bg-[var(--bg-surface)] p-2 font-mono text-[11px] leading-5 outline-none focus:ring-1 focus:ring-[var(--accent)]"
            value={d.editedText}
          ...
          {d.saveRevision.isError && (
            <p className="mt-1 text-[11px] text-[var(--danger)]">{d.saveRevision.error.message}</p>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-2 p-1">
      ...
      >
        ...
        {d.graphQ.isError && (
          <p className="text-[12px] text-[var(--danger)]">{d.graphQ.error?.message}</p>
        )}
        {!d.graphQ.data && !d.graphQ.isLoading && !d.graphQ.isError && (
          <EmptyState title="Sin relaciones" description="No se detectaron relaciones." />
        )}
      </CollapsibleCard>
    </div>
  )
          ...
          {d.timelineEvents.length > 0 ? (
            <div className="space-y-0.5">
              {d.timelineEvents.map((e, i) => (
                <TimelineEventRow key={e.id} event={e} isLast={i === d.timelineEvents.length - 1} />
              ))}
            </div>
          ) : (
            <div className="space-y-0.5">
              <TimelineEventRow
                ...
                isLast={!d.document?.processed_at}
              />
