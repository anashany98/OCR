...
// DocumentHeader
// ---------------------------------------------------------------------------
export function DocumentHeader({ d }: { d: DocumentDetail }) {
...
function ActionToolbar({ d }: { d: DocumentDetail }) {
    ...
    <div className="flex flex-wrap gap-1.5">
      ...
      <PermissionGate roles={["admin", "gestor"]}>
        <Button
          ...
          size="sm"
          className="h-8 text-xs"
          // P0.2: the "Corregir tipo" flow is not implemented in the
          // backend yet (no PATCH /documents/{id}/type endpoint).
          // Show a toast instead of a native ``window.alert`` so
          // the UI stays consistent and accessible.
          onClick={() =>
            notify.info(
              "Funcionalidad pendiente",
              "La corrección de tipo de documento se implementará en un próximo sprint.",
            )
          }
        >
      ...
      <PermissionGate roles={["admin", "gestor"]}>
        <Button
          ...
          size="sm"
          className="h-8 text-xs"
          // P0.2: the "Enviar a revisión" flow is not implemented
          // yet. The button stays in the UI to signal the
          // intended action; clicking it tells the operator the
          // feature is on the roadmap. ``window.alert`` was
          // jarring and inconsistent with the rest of the UI.
          onClick={() =>
            notify.info(
              "Funcionalidad pendiente",
              "El flujo de envío a revisión se implementará en un próximo sprint.",
            )
          }
        >
...
// ViewerCard
// ---------------------------------------------------------------------------
export function ViewerCard({ d }: { d: DocumentDetail }) {
  const document = d.document
...
// OcrCard
// ---------------------------------------------------------------------------
export function OcrCard({ d }: { d: DocumentDetail }) {
  const page = d.selectedPage
  return (
    <Card className="overflow-hidden">
...
function OcrRevisionEditor({ d }: { d: DocumentDetail }) {
  const page = d.selectedPage
  if (!page) return null
  return (
...
// KeyEntitiesCard
// ---------------------------------------------------------------------------
export function KeyEntitiesCard({ entities }: { entities: DocumentDetail["keyEnts"] }) {
  return (
    <Card>
...
// TimelineCard
// ---------------------------------------------------------------------------
export function TimelineCard({ d }: { d: DocumentDetail }) {
  const id = d.document?.id
  return (
    <Card>
      <CardHeader className="pb-3">
...
// ---------------------------------------------------------------------------
export function BelowSection({ d }: { d: DocumentDetail }) {
  return (
