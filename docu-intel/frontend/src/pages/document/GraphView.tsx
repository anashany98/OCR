...
const RELATION_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  presupuesto_pedido: {
    ...
    text: "text-[var(--text-on-info)]",
    border: "border-[var(--info)]/30",
  },
  pedido_factura: {
    bg: "bg-[var(--success-light)]",
    text: "text-[var(--text-on-success)]",
    border: "border-[var(--success)]/30",
  },
  presupuesto_factura: {
    bg: "bg-[var(--warning-light)]",
    text: "text-[var(--text-on-warning)]",
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <Badge variant="neutral" className="gap-1">
          <FileText className="h-3 w-3" /> {graph.nodes.length} documentos
        </Badge>
        <Badge variant="info" className="gap-1">
          <Link2 className="h-3 w-3" /> {graph.edges.length} relaciones
        </Badge>
        <Badge variant="success" className="gap-1">
          <GitBranch className="h-3 w-3" /> {edgesByRelation.size} tipos
        </Badge>
      </div>
            ...
            {graph.edges.length > 0 && (
              <div className="flex flex-wrap items-start justify-center gap-3">
                {graph.edges.map((edge, i) => {
                  ...
                  const connectedId =
                    edge.from_document_id === currentDocId
                      ? edge.to_document_id
                    ...
                    <div key={i} className="flex flex-col items-center gap-1.5">
                      ...
                      <Link
                        ...
                        className={cn(
                          ...
                          connectedId === currentDocId
                            ? "border-[var(--accent)] bg-[var(--accent-light)]"
                            : "border-[var(--border)] bg-[var(--bg-surface)] hover:border-[var(--border-2)]",
                        )}
                      >
                        <div
                          className={cn(
                        ...
                        <p className="max-w-[100px] truncate text-[10px] font-medium text-[var(--text-primary)]">
                          {connectedNode.filename}
                        </p>
                        <p className="text-[9px] text-[var(--text-muted)]">
                          {connectedNode.document_type}
                        </p>
                        {edge.label && (
                          <p className="max-w-[100px] truncate text-[9px] text-[var(--text-muted)] italic">
                            &quot;{edge.label}&quot;
                          </p>
                        )}
                      </Link>
                    ...
                    {edges.map((edge, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-2 text-[11px]">
                          ...
                          {edge.label && (
                            <span className="text-[var(--text-muted)] italic">({edge.label})</span>
                          )}
                        </div>
                      )
                    })}
                  </div>
                  {relation !== Array.from(edgesByRelation.keys()).slice(-1)[0] && (
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-[13px]">
            Documentos relacionados ({graph.nodes.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
