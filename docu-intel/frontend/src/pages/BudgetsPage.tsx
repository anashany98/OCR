...
export function BudgetsPage() {
  ...
  const acceptedWithoutOrder = useQuery({
    queryKey: ["budgets", "accepted-without-order"],
    queryFn: api.acceptedBudgetsWithoutOrder,
  })
  ...
  return (
    <>
      ...
      <PageHeader
        title="Presupuestos"
        description="Extracción básica de cabecera, estado y líneas detectadas."
      />
      <Card>
        <CardHeader>
    ...
    <Table>
      ...
      <TableBody>
        {rows.map((budget) => (
          <TableRow
            key={budget.id}
            className={budget.id === selectedId ? "bg-muted/60" : undefined}
          >
            ...
            <TableCell>
              {budget.confidence ? `${Math.round(budget.confidence * 100)}%` : "-"}
            </TableCell>
