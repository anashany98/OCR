...
export function UrgentActionCard({ action }: { action: UrgentAction }) {
  ...
  const { ring, accentText, accentIcon, badgeBg } = (() => {
    switch (action.tone) {
      case "danger": return { ring: "border-[var(--danger)]/20 bg-[var(--danger-faint)]", accentText: "text-[var(--text-on-danger)]", accentIcon: "text-[var(--danger)]", badgeBg: "bg-[var(--danger)]" }
      case "warning": return { ring: "border-[var(--warning)]/20 bg-[var(--warning-faint)]", accentText: "text-[var(--text-on-warning)]", accentIcon: "text-[var(--warning)]", badgeBg: "bg-[var(--warning)]" }
      default: return { ring: "border-[var(--info)]/20 bg-[var(--info-faint)]", accentText: "text-[var(--text-on-info)]", accentIcon: "text-[var(--info)]", badgeBg: "bg-[var(--info)]" }
    }
  })()

  return (
    <Link to={action.to} className={cn("group relative flex flex-col gap-1.5 rounded-lg border p-3.5 transition-all hover:shadow-sm", ring)}>
      <div className="flex items-start justify-between">
        <Icon className={cn("h-4 w-4", accentIcon)} aria-hidden="true" />
        {action.count != null && action.count > 0 && (
          <span className={cn("flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-[10px] font-semibold text-white", badgeBg)}>
  ...
  return (
    <section>
      ...
      <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4">
        {actions.map((a) => <UrgentActionCard key={a.to + a.label} action={a} />)}
      </div>
    </section>
  )
}
...
export function MetricStrip({
  ...
  return (
    <section>
      <h2 className="mb-2 text-[13px] font-semibold text-[var(--text-primary)]">Operación de hoy</h2>
      <div className="grid gap-2.5 sm:grid-cols-3 lg:grid-cols-6">
        <MetricTile title="Procesados" value={isLoading ? "—" : (stats?.documents_processed ?? 0)} meta={`${stats?.documents_total ?? 0} totales`} tone="success" icon={<CheckCircle2 className="h-4 w-4" />} />
        <MetricTile title="En cola" value={ov?.jobs.pending_or_processing ?? "—"} meta={`${formatEta(undefined)} restantes`} tone={(ov?.jobs.pending_or_processing ?? 0) > 0 ? "warning" : "neutral"} icon={<Clock className="h-4 w-4" />} />
        <MetricTile title="Fallidos" value={isLoading ? "—" : (stats?.documents_failed ?? 0)} meta="Requieren atención" tone={(stats?.documents_failed ?? 0) > 0 ? "danger" : "success"} icon={<AlertTriangle className="h-4 w-4" />} />
        <MetricTile title="Revisión" value={isLoading ? "—" : (stats?.documents_needs_review ?? 0)} meta="Sin cola" tone={(stats?.documents_needs_review ?? 0) > 0 ? "warning" : "neutral"} icon={<FileWarning className="h-4 w-4" />} />
        <MetricTile title="Pipeline" value={ov?.jobs.pending_or_processing ?? "—"} meta="Workers" tone="info" icon={<Workflow className="h-4 w-4" />} />
        <MetricTile title="Incidencias" value={inboxCount} meta={criticalInboxCount > 0 ? `${criticalInboxCount} críticas` : "Sin críticas"} tone={criticalInboxCount > 0 ? "danger" : "success"} icon={<Inbox className="h-4 w-4" />} />
      </div>
    </section>
  )
}
    ...
    <Card>
      <CardHeader className="flex-row items-baseline justify-between pb-3">
        ...
        <Button asChild variant="ghost" size="sm" className="text-[11px] text-[var(--accent)] h-7">
          <Link to="/work-inbox">Ver todas <ArrowRight className="ml-1 h-3 w-3" /></Link>
        </Button>
      </CardHeader>
      <CardContent className="px-0 py-0">
        {inboxItems.length === 0 ? (
          <div className="flex flex-col items-center gap-2 py-10 text-center">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--success-faint)] text-[var(--success)]"><CheckCircle2 className="h-5 w-5" /></div>
            <p className="text-[13px] font-medium text-[var(--text-primary)]">Sin trabajo pendiente</p>
          </div>
        ) : (
          <ul className="divide-y divide-[var(--border)]">
            {inboxItems.slice(0, 6).map((item, i) => (
              <li key={`${item.kind}-${item.document_id ?? "d"}-${i}`}>
                <Link to={item.action_url || "/work-inbox"} className="group flex items-center gap-3 px-5 py-3 hover:bg-[var(--bg-surface-2)]/60 transition-colors">
                  <span className={cn("h-1.5 w-1.5 rounded-full flex-shrink-0",
                    item.severity === "error" || item.severity === "critical" ? "bg-[var(--danger)]" :
                    item.severity === "warning" ? "bg-[var(--warning)]" : "bg-[var(--info)]"
                  )} />
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[12px] font-medium text-[var(--text-primary)]">{item.title}</p>
...
export function DistributionCard({
  ...
  metrics: { documents_by_status?: Record<string, number>; documents_by_type?: Record<string, number>; jobs_by_status?: Record<string, number> } | undefined
}) {
  return (
    <Card>
...
function DistributionColumn({ title, values }: { title: string; values?: Record<string, number> }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-[10px] font-semibold uppercase tracking-wider text-[var(--text-muted)]">{title}</p>
          ...
          return (
            <div key={key} className="space-y-0.5">
              <div className="flex items-center justify-between text-[11px]">
                <span className="truncate capitalize text-[var(--text-secondary)]">{key.replace(/_/g, " ")}</span>
...
export function InfrastructureCard({
  ...
  ov: { disk?: { input_dir?: { total: number; used: number; free: number }; files_dir?: { total: number; used: number; free: number } } } | undefined
}) {
  return (
    <Card>
      <CardHeader className="pb-3">
      ...
      <CardContent className="space-y-1.5">
        ...
        <InfraRow label="Disco entrada" status={getDiskStatus(ov?.disk?.input_dir)} detail={formatDiskSpace(ov?.disk?.input_dir)} />
        <InfraRow label="Disco archivos" status={getDiskStatus(ov?.disk?.files_dir)} detail={formatDiskSpace(ov?.disk?.files_dir)} />
      </CardContent>
    </Card>
  )
}

function InfraRow({ label, status, detail }: { label: string; status: InfraStatus; detail?: string }) {
  const dotColor = status === "ok" ? "bg-[var(--success)]" : status === "warning" ? "bg-[var(--warning)]" : status === "error" ? "bg-[var(--danger)]" : "bg-[var(--text-muted)]"
  const badgeVariant = status === "ok" ? "success" : status === "warning" ? "warning" : status === "error" ? "danger" : "neutral"
  return (
    <div className="flex items-center justify-between rounded-md px-2.5 py-1.5 text-[12px]">
      ...
      <div className="flex items-center gap-2">
        {detail && <span className="font-mono text-[10px] tabular-nums text-[var(--text-muted)]">{detail}</span>}
  ...
  return (
    <Card>
      <CardHeader className="flex-row items-baseline justify-between pb-3">
        ...
        {alertItems.length > 0 && <Badge variant="warning" className="text-[10px]">{alertItems.length}</Badge>}
      </CardHeader>
      <CardContent className="px-0 py-0">
        {alertItems.length === 0 ? (
          <p className="py-6 text-center text-[11px] text-[var(--text-muted)]">Sin alertas activas</p>
        ) : (
          <ul className="divide-y divide-[var(--border)]">
            {alertItems.slice(0, 4).map((a) => (
              <li key={a.key} className="flex items-start justify-between gap-3 px-5 py-2.5">
                ...
                <Badge variant={a.severity === "critical" ? "danger" : a.severity === "warning" ? "warning" : "info"} className="text-[10px] flex-shrink-0">{a.count}</Badge>
              </li>
            ))}
          </ul>
        )}
...
function ShortcutLink({ to, icon: Icon, label }: { to: string; icon: LucideIcon; label: string }) {
  return (
    <Button asChild variant="ghost" size="sm" className="w-full justify-between text-[12px] h-8">
      <Link to={to}>
        <span className="flex items-center gap-2"><Icon className="h-3.5 w-3.5 text-[var(--text-muted)]" /> {label}</span>
