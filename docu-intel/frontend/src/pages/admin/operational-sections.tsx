    ...
    <>
      ...
      <Card>
        <CardContent className="space-y-2 pt-4">
          ...
          {!alerts.length && (
            <p className="text-sm text-[var(--text-muted)]">Sin alertas operativas activas.</p>
          )}
        </CardContent>
      </Card>
    </>
            ...
            <Button
              ...
              onClick={async () => {
                const ok = await confirm({
                  ...
                  description: "Se detendrán los nuevos escaneos.",
                  confirmLabel: "Pausar",
                  tone: "danger",
                })
            ...
            <Button
              ...
              onClick={async () => {
                const ok = await confirm({
                  ...
                  description: "Se reanudarán los escaneos.",
        ...
        <Card>
          <CardContent className="flex flex-col gap-2 pt-4 text-sm">
            {inputFolders.map((folder) => (
              <code key={folder} className="rounded-md bg-[var(--bg-surface-2)] px-2 py-1 text-xs">
                {folder}
              </code>
            ))}
          </CardContent>
        ...
        <CardContent className="pt-4">
          <form className="grid gap-3 md:grid-cols-5" onSubmit={onReprocessSubmit}>
            ...
            <Button disabled={reprocessPending || (!status && !documentType && !sourcePath)}>
              <RefreshCw data-icon="inline-start" /> Reprocesar
            </Button>
          </form>
          {reprocessResult && (
          ...
          <CardContent className="space-y-3 pt-4">
            ...
            >
              ...
              <Button disabled={loadDocumentGraph.isPending}>
                <Network data-icon="inline-start" /> Cargar grafo
              </Button>
            </form>
            ...
            {loadDocumentGraph.isError && (
              <p className="text-sm text-[var(--danger)]">{loadDocumentGraph.error?.message}</p>
            )}
          </CardContent>
        </Card>
      </div>
