...
export function ChainsHotelsSection({
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      <Card>
        <CardHeader><CardTitle>Cadenas</CardTitle></CardHeader>
        <CardContent className="space-y-3">
          <form className="flex gap-2" onSubmit={(e: FormEvent) => { e.preventDefault(); if (chainName.trim()) createChain.mutate() }}>
            <Input value={chainName} onChange={(e) => setChainName(e.target.value)} placeholder="Nombre de cadena" />
            <Button disabled={createChain.isPending}>Crear</Button>
          </form>
          <SimpleTable rows={chains.map((c) => [c.name, c.is_active ? "Activa" : "Inactiva"])} headings={["Cadena", "Estado"]} />
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <SimpleTable headings={["Hotel", "Cadena", "Código"]} rows={hotels.map((h) => [h.name, chains.find((c) => c.id === h.chain_id)?.name ?? String(h.chain_id), h.code ?? "-"])} />
        </CardContent>
      </Card>
    </div>
  )
...
export function FolderRulesSection({
  chains, hotels, folderRules,
  ruleName, setRuleName, rulePattern, setRulePattern,
  ruleChainId, setRuleChainId, ruleHotelId, setRuleHotelId,
  ruleTags, setRuleTags, createFolderRule, applyFolderRules,
}: FolderRulesSectionProps) {
  return (
    <Card>
      <CardHeader><CardTitle>Reglas de carpetas</CardTitle></CardHeader>
      <CardContent className="space-y-3">
        ...
        <SimpleTable headings={["Patrón", "Cadena", "Hotel", "Tags", "Estado"]} rows={folderRules.map((r) => [
          r.pattern,
...
export function AccessGroupsSection({
  accessGroups, tenantAdminEnabled,
  groupName, setGroupName, groupChainIds, setGroupChainIds,
  groupHotelIds, setGroupHotelIds, groupDeniedTags, setGroupDeniedTags,
  groupAllowAll, setGroupAllowAll, groupCanPrices, setGroupCanPrices,
  groupCanSearchBudgets, setGroupCanSearchBudgets, createAccessGroup,
}: AccessGroupsSectionProps) {
  return (
    <Card>
      <CardHeader><CardTitle>Perfiles y grupos</CardTitle></CardHeader>
      <CardContent className="space-y-3">
        <form className="grid gap-2" onSubmit={(e: FormEvent) => { e.preventDefault(); if (groupName.trim()) createAccessGroup.mutate() }}>
          ...
          <div className={tenantAdminEnabled ? "grid gap-2 md:grid-cols-3" : "grid gap-2"}>
            {tenantAdminEnabled && (
              <>
                <Input value={groupChainIds} onChange={(e) => setGroupChainIds(e.target.value)} placeholder="IDs cadena: 1,2" />
                <Input value={groupHotelIds} onChange={(e) => setGroupHotelIds(e.target.value)} placeholder="IDs hotel: 3,4" />
              </>
            )}
            <Input value={groupDeniedTags} onChange={(e) => setGroupDeniedTags(e.target.value)} placeholder="tags bloqueados" />
          </div>
          <div className="flex flex-wrap gap-3 text-sm">
            ...
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={groupCanPrices} onChange={(e) => setGroupCanPrices(e.target.checked)} />
              Ver precios
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={groupCanSearchBudgets} onChange={(e) => setGroupCanSearchBudgets(e.target.checked)} />
              Buscar presupuestos
            </label>
          </div>
          <Button disabled={createAccessGroup.isPending}>Crear grupo</Button>
        </form>
        <SimpleTable headings={["Grupo", "Permisos", "Tags bloqueados"]} rows={accessGroups.map((g) => [
          g.name,
          [g.permissions_json.can_view_prices ? "precios" : "sin precios", g.permissions_json.can_search_budgets ? "busca presupuestos" : "búsqueda limitada", tenantAdminEnabled ? (g.permissions_json.allow_all_hotels ? "todos hoteles" : `hoteles: ${String(g.permissions_json.hotel_ids ?? "[]")}`) : null].filter(Boolean).join(" · "),
...
export function MembersSection({
  accessGroups, memberGroupId, setMemberGroupId,
  memberType, setMemberType, memberPrincipalId, setMemberPrincipalId, upsertMember,
}: MembersSectionProps) {
  return (
    <Card>
...
export function SensitiveTagsSection({
  sensitiveTags, tagName, setTagName, tagDescription, setTagDescription, createSensitiveTag,
}: SensitiveTagsSectionProps) {
  return (
    <Card>
      <CardHeader><CardTitle>Tags sensibles</CardTitle></CardHeader>
      <CardContent className="space-y-3">
        <form className="grid gap-2 md:grid-cols-[220px_1fr_auto]" onSubmit={(e: FormEvent) => { e.preventDefault(); if (tagName.trim()) createSensitiveTag.mutate() }}>
          <Input value={tagName} onChange={(e) => setTagName(e.target.value)} placeholder="contabilidad" />
          <Input value={tagDescription} onChange={(e) => setTagDescription(e.target.value)} placeholder="Descripción" />
          <Button disabled={createSensitiveTag.isPending}>Crear tag</Button>
        </form>
        <SimpleTable headings={["Tag", "Descripción", "Estado"]} rows={sensitiveTags.map((t) => [t.name, t.description ?? "-", t.is_active ? "Activo" : "Inactivo"])} />
      </CardContent>
    </Card>
  )
}
...
export function AccessSimulatorSection({
  explainPrincipalType, setExplainPrincipalType, explainPrincipalId, setExplainPrincipalId,
  explainDocumentId, setExplainDocumentId, explainAccess,
}: AccessSimulatorSectionProps) {
  return (
    <Card>
        ...
        {explainAccess.data && (
          <div className="rounded-md border p-3 text-sm">
            ...
            <ul className="mt-2 space-y-1 text-[var(--text-muted)]">
              {explainAccess.data.reasons.map((reason: string) => <li key={reason}>{reason}</li>)}
            </ul>
          </div>
        )}
        {explainAccess.isError && <p className="text-sm text-[var(--danger)]">{explainAccess.error?.message}</p>}
      </CardContent>
    </Card>
  )
}
...
export function RulePreviewSection({
  rulePreviewPath, setRulePreviewPath, rulePreviewPattern, setRulePreviewPattern,
  rulePreviewTags, setRulePreviewTags, previewRule,
}: RulePreviewSectionProps) {
  return (
    <Card>
      <CardHeader><CardTitle>Preview reglas</CardTitle></CardHeader>
      <CardContent className="space-y-2 text-sm">
        <form className="grid gap-2" onSubmit={(e: FormEvent) => { e.preventDefault(); if (rulePreviewPath.trim() && rulePreviewPattern.trim()) previewRule.mutate() }}>
          <Input value={rulePreviewPath} onChange={(e) => setRulePreviewPath(e.target.value)} placeholder="Ruta de archivo" />
          <Input value={rulePreviewPattern} onChange={(e) => setRulePreviewPattern(e.target.value)} placeholder="/presupuestos/" />
          <Input value={rulePreviewTags} onChange={(e) => setRulePreviewTags(e.target.value)} placeholder="tags separados por coma" />
        ...
        {previewRule.data && (
          <div className="rounded-md border p-2">
            ...
            <p className="mt-2 text-xs text-[var(--text-muted)]">{previewRule.data.normalized_path}</p>
          </div>
        )}
        {previewRule.isError && <p className="text-sm text-[var(--danger)]">{previewRule.error?.message}</p>}
      </CardContent>
    </Card>
  )
}
...
export function RedactionPreviewSection({
  redactionPrincipalType, setRedactionPrincipalType, redactionPrincipalId, setRedactionPrincipalId,
  redactionText, setRedactionText, previewRedaction,
}: RedactionPreviewSectionProps) {
  return (
    <Card>
      ...
      <CardContent className="space-y-2 text-sm">
        ...
        {previewRedaction.isError && <p className="text-sm text-[var(--danger)]">{previewRedaction.error?.message}</p>}
      </CardContent>
    </Card>
  )
}
