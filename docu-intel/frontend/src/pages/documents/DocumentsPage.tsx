...
import { Download, Eye, FolderUp, Loader2, RefreshCcw, Search, Upload, X } from "lucide-react"
...
export function DocumentsPage() {
  ...
  const upload = useMutation({
    ...
    onSuccess: () => {
      ...
      notify.success("Subido", "Encolado para procesamiento.")
    },
    onError: (e) => notify.error(e, "Error al subir"),
  })
  const uploadBatch = useMutation({
    mutationFn: (p: { files: File[]; relativePaths?: string[] }) =>
      api.uploadBatch(p.files, p.relativePaths),
    onSuccess: (d) => {
      qc.invalidateQueries({ queryKey: ["documents"] })
      notify.success(
        "Subida completada",
        `${d.uploaded} nuevos, ${d.duplicates} duplicados, ${d.failed} fallidos.`,
      )
    },
    onError: (e) => notify.error(e, "Error al subir"),
  })
  const scan = useMutation({
    ...
    onSuccess: (d) => {
      qc.invalidateQueries({ queryKey: ["documents"] })
      notify.success("Escaneo", `${d.registered} nuevos, ${d.duplicates} duplicados.`)
    },
  ...
  const reprocess = useMutation({
    ...
    onSuccess: (j) => {
      qc.invalidateQueries({ queryKey: ["documents"] })
      notify.success("Reprocesado", `Job #${j.id}`)
    },
  ...
  const bulkReprocess = useMutation({
    ...
    onSuccess: (d) => {
      setSelected([])
      qc.invalidateQueries({ queryKey: ["documents"] })
      notify.success("Lote", `${d.enqueued} encolados`)
    },
    onError: (e) => notify.error(e, "Error"),
  })
  ...
  function changeView(v: DocumentViewId) {
    setView(v)
    setStatus("")
    setDocType("")
    setOffset(0)
  ...
  function toggleSel(id: number) {
    setSelected((c) => (c.includes(id) ? c.filter((i) => i !== id) : [...c, id]))
  }
  function togglePage() {
    const ids = rows.map((d) => d.id)
    const all = ids.length > 0 && ids.every((id) => selSet.has(id))
    setSelected((c) =>
      all ? c.filter((id) => !ids.includes(id)) : Array.from(new Set([...c, ...ids])),
    )
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-[12px] text-[var(--text-muted)]">
            {total} documentos · Filtros de servidor, vistas guardadas y acciones masivas.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          >
            <RefreshCcw className="mr-1 h-3 w-3" /> Escanear
          </Button>
          ...
          <Button variant="outline" size="sm" asChild>
            <label className="cursor-pointer">
              <FolderUp className="mr-1 h-3 w-3" /> Carpeta
              <input
                {...({
                  className: "hidden",
                  type: "file",
      ...
      {dragging && (
        <div className="flex items-center justify-center rounded-lg border-2 border-dashed border-[var(--accent)] bg-[var(--accent-light)] py-6">
          <Upload className="mr-2 h-5 w-5 text-[var(--accent)]" />
          <span className="text-[13px] font-medium text-[var(--accent)]">Suelta archivos</span>
        </div>
      )}
      ...
      {(upload.isError || uploadBatch.isError) && (
        ...
        >
          Error: {upload.error?.message ?? uploadBatch.error?.message}
        </Banner>
      )}
      {upload.isSuccess && (
        <Banner tone="success" onDismiss={() => upload.reset()}>
          Archivo subido.
        </Banner>
      )}
      {uploadBatch.isSuccess && uploadBatch.data && (
        <Banner tone="success" onDismiss={() => uploadBatch.reset()}>
          Subidos: {uploadBatch.data.uploaded} nuevos, {uploadBatch.data.duplicates} dup,{" "}
          {uploadBatch.data.failed} fallos.
        </Banner>
      )}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select
          ...
          className="h-8 rounded-md border border-[var(--border-2)] bg-[var(--bg-surface)] px-2 text-[12px] text-[var(--text-primary)]"
        >
        ...
        <CardContent className="p-0">
          {docs.isLoading ? (
            <div className="space-y-0">
              {Array.from({ length: 8 }).map((_, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 border-b border-[var(--border)] px-3 py-2.5"
                >
            ...
            <Table>
                ...
                <TableRow>
                  <TableHead className="w-8">
                    <input
                      type="checkbox"
                      aria-label="Todo"
                      checked={rows.length > 0 && rows.every((d) => selSet.has(d.id))}
                      onChange={togglePage}
                    />
                  </TableHead>
              ...
              <TableBody>
                {rows.map((doc) => (
                  <TableRow
                    key={doc.id}
                    className={cn(selected.includes(doc.id) && "bg-[var(--accent-faint)]")}
                  >
                    <TableCell>
                      <input
                        type="checkbox"
                        checked={selSet.has(doc.id)}
                    ...
                    <TableCell>
                      ...
                      <p className="truncate text-[10px] text-[var(--text-muted)]">
                        {doc.source_path ?? doc.file_hash?.slice(0, 16)}
                      </p>
                    </TableCell>
                    <TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={7}>
                      <EmptyState
                        ...
                        description="Sube archivos o escanea carpetas."
                      />
                    </TableCell>
                  </TableRow>
                )}
          ...
          <div className="flex items-center justify-between border-t border-[var(--border)] px-4 py-2 text-[11px]">
            <span className="text-[var(--text-muted)]">
              {rows.length ? offset + 1 : 0}–{offset + rows.length} de {total}
            </span>
...
function Banner({
  ...
  const c = {
    ...
    success: "border-[var(--success)]/20 bg-[var(--success-faint)] text-[var(--text-on-success)]",
  }
  return (
    <div
      className={cn(
