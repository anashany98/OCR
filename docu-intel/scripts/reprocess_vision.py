...
# Monitor
print("\nMonitoring (4 workers in parallel)...")
start = time.time()
...
result3 = subprocess.run(
    ["docker", "exec", "docu-intel-postgres-1", "psql", "-U", "app", "-d", "docuintel", "-t", "-A", "-c",
     "SELECT dp.ocr_confidence, COUNT(*) FROM document_pages dp JOIN documents d ON d.id = dp.document_id WHERE d.id IN (SELECT DISTINCT d2.id FROM document_pages dp2 JOIN documents d2 ON d2.id = dp2.document_id WHERE dp2.ocr_confidence = 0 AND d2.status != 'duplicate') GROUP BY dp.ocr_confidence ORDER BY dp.ocr_confidence;"],
    capture_output=True, encoding="utf-8", timeout=15
)
print(f"\nFinal OCR confidence distribution:")
