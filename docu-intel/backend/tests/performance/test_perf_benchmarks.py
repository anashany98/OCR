...
# The persisted vectors are Vector(768); synthetic benchmark data must honour
# the schema rather than the former 1,024-dimension local-model default.
...
def _fabricate(sess):
    ...
    for i in range(100):
        d = Document(
            ...
            stored_filename=f"aa/doc-{i}.txt",
            source_path=f"/data/input/doc-{i}.txt",
            file_hash=_hash(i),
            extension=".txt",
            file_size=1000,
            document_type="desconocido",
            status=random.choice(["processed", "pending", "needs_review"]),
    ...
    for i in range(30):
        ...
        sess.add(
            Budget(
                document_id=d.id,
                budget_number=f"2026/{100 + i}",
                client_name="Cliente",
                date=datetime.utcnow().date(),
                total_amount=100.0,
                currency="EUR",
                accepted_detected=i < 10,
            )
        )
    sess.commit()
...
def test_search_semantic_scales_to_1000_chunks(monkeypatch):
    ...
    assert p95 < 30.0, (
        f"semantic search P95={p95:.3f}s exceeds 30s (SQLite without pgvector; production needs pgvector)"
    )
    sess.close()
...
def test_admin_listing_uses_pagination():
    ...
    total = sess.scalar(
        select(text("count(*)")).select_from(Document).where(Document.deleted_at.is_(None))
    )
    assert total == 100
    sess.close()
...
def test_document_pages_endpoint_has_pagination_guard():
    ...
    d = Document(
        ...
        stored_filename="aa/big.pdf",
        source_path="/data/input/big.pdf",
        file_hash="a" * 64,
        extension=".pdf",
        file_size=5000,
        document_type="plano",
    ...
    # Without limit
    all_pages = list(
        sess.scalars(select(DocumentPage).where(DocumentPage.document_id == d.id)).all()
    )
    assert len(all_pages) == 50
    # With limit
    limited = list(
        sess.scalars(select(DocumentPage).where(DocumentPage.document_id == d.id).limit(10)).all()
    )
...
def test_postgres_indexes_defined_on_critical_columns():
    ...
    assert DocumentEntity.__table__.columns["normalized_value"].index is not None or any(
        col.name == "normalized_value"
        for idx in DocumentEntity.__table__.indexes
