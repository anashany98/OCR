...
class ClassificationSuggestion(Base):
    ...
    document = relationship(
        "Document", foreign_keys=[document_id], back_populates="classification_suggestions"
    )
...
class LearnedPattern(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "pattern_type", "pattern_value", "target_action", name="uq_learned_patterns_value"
        ),
    )
    ...
    source_suggestion_id: Mapped[int | None] = mapped_column(
        ForeignKey("classification_suggestions.id", ondelete="SET NULL"), nullable=True
    )
    status: Mapped[str] = mapped_column(String(20), nullable=False, default="pending", index=True)
    ...
    source_suggestion = relationship(
        "ClassificationSuggestion", foreign_keys=[source_suggestion_id]
    )
