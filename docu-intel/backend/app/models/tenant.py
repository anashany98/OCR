...
class Hotel(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    chain_id: Mapped[int] = mapped_column(ForeignKey("hotel_chains.id", ondelete="CASCADE"), index=True, nullable=False)
...
class DocumentAccessMetadata(Base):
    ...
    chain_id: Mapped[int | None] = mapped_column(ForeignKey("hotel_chains.id", ondelete="SET NULL"), index=True)
    hotel_id: Mapped[int | None] = mapped_column(ForeignKey("hotels.id", ondelete="SET NULL"), index=True)
    assignment_status: Mapped[str] = mapped_column(String(50), default="quarantine", nullable=False, index=True)
...
class AccessGroupMember(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    group_id: Mapped[int] = mapped_column(ForeignKey("access_groups.id", ondelete="CASCADE"), index=True, nullable=False)
