...
class DocumentOccurrence(Base):
    ...
    __table_args__ = (UniqueConstraint("source_root", "source_path", name="uq_occurrence_source"),)
    ...
    year: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    brand_id: Mapped[int] = mapped_column(
        ForeignKey("hotel_chains.id", ondelete="SET NULL"),
        index=True,
        nullable=False,
    )
    hotel_id: Mapped[int | None] = mapped_column(
    ...
    folder_budget_code: Mapped[str | None] = mapped_column(String(120), index=True)
    document_budget_code: Mapped[str | None] = mapped_column(String(120), index=True)
    resolved_budget_code: Mapped[str | None] = mapped_column(String(120), index=True)
    association_status: Mapped[str] = mapped_column(
        String(30), default="folder_only", nullable=False, index=True
    )  # verified | folder_only | content_only | conflict | manual
    association_evidence: Mapped[dict[str, Any] | None] = mapped_column(JSON)
...
class DocumentBudgetLink(Base):
    ...
    budget_scope_id: Mapped[int] = mapped_column(
        ForeignKey("budget_scopes.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    )
