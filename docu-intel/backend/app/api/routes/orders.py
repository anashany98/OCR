...
def get_order(order_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> Order:
    order = db.get(Order, order_id)
    if not order or order not in filter_records_by_document_scope(db, [order], resolve_user_access_scope(db, user)):
        raise HTTPException(status_code=404, detail="Order not found")
    return order


@router.get("/{order_id}/lines", response_model=list[OrderLineRead])
def get_order_lines(order_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> list[OrderLine]:
    order = db.get(Order, order_id)
    if not order or order not in filter_records_by_document_scope(db, [order], resolve_user_access_scope(db, user)):
        raise HTTPException(status_code=404, detail="Order not found")
    return list(db.scalars(select(OrderLine).where(OrderLine.order_id == order_id).order_by(OrderLine.id.asc())).all())
