...
def explain_document_access(
    db: Session, *, principal_type: str, principal_id: str, document: Document
) -> dict:
    ...
    if document.deleted_at is not None:
        return {
            "allowed": False,
            "reasons": ["Documento borrado logicamente"],
            "scope": _scope(scope),
        }
    if scope.is_admin:
    ...
    if not metadata:
        if scope.allow_unassigned_documents:
            return {
                "allowed": True,
                "reasons": ["Perfil permite documentos sin asignacion"],
                "scope": _scope(scope),
            }
        return {
            "allowed": False,
            "reasons": ["Documento sin metadatos de acceso"],
            "scope": _scope(scope),
        }
    ...
    if metadata.assignment_status != "assigned":
        if scope.allow_unassigned_documents:
            return {
                "allowed": True,
                "reasons": [f"Perfil permite documento en estado {metadata.assignment_status}"],
                "scope": _scope(scope),
            }
        return {
            "allowed": False,
            "reasons": [f"Documento en estado {metadata.assignment_status}"],
            "scope": _scope(scope),
        }
    ...
    if scope.allow_all_hotels:
        return {
            "allowed": True,
            "reasons": ["Perfil permite todos los hoteles"],
            "scope": _scope(scope),
        }
    if metadata.hotel_id and metadata.hotel_id in scope.hotel_ids:
        return {
            "allowed": True,
            "reasons": [f"Hotel autorizado: {metadata.hotel_id}"],
            "scope": _scope(scope),
        }
    if metadata.chain_id and metadata.chain_id in scope.chain_ids:
        return {
            "allowed": True,
            "reasons": [f"Cadena autorizada: {metadata.chain_id}"],
            "scope": _scope(scope),
        }
    return {
        "allowed": False,
        "reasons": ["Fuera del scope de cadena/hotel autorizado"],
        "scope": _scope(scope),
    }
