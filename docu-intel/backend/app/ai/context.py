...
def build_grounded_response(
    *,
    ...
    The text uses a structured layout (secciones ``Respuesta:``,
    ``Datos:``, ``Fuentes:``, ``Confianza:`` y ``Advertencias:``)
    so the system prompt and the LLM can both produce
    machine-checkable answers. When there is no context the
    answer explicitly says it cannot confirm anything and the
    confidence drops to ``0.0``; when there is context the
    sections are filled in from the top hits.

    Contract: the four required sections must always be present
    in the answer string (see ``app.ai.validation.has_required_sections``).
    Test coverage: ``tests/test_phase3_ai_search.py::test_grounded_response_uses_required_sections_and_refuses_without_data``.
    """
    if not context_items:
        ...
        answer = (
            "Respuesta: No puedo confirmarlo con la informacion disponible. "
            "No he encontrado datos suficientes en los documentos para responder "
            "a tu pregunta con seguridad.\n\n"
            "Datos: -\n\n"
            "Fuentes: -\n\n"
            "Confianza: 0.0\n\n"
            "Advertencias:\n"
            f"{warning_text}\n\n"
            "_Si me das mas contexto (numero de presupuesto, proveedor, ejercicio, "
            "etc.) o subes el documento relevante, lo reviso de nuevo._"
        )
    ...
    # Build the structured sections: ``Datos`` (the top quote), ``Fuentes``
    # (up to 3 extra documents the user can jump to), ``Confianza`` (the
    # mean OCR confidence) and ``Advertencias`` (low-OCR notice, etc.).
    datos = quote or "-"
    ...
    fuentes = "\n".join(fuentes_lines) if fuentes_lines else "-"

    advertencia_lines = list(warnings) if warnings else []
    advertencias = "\n".join(f"- {a}" for a in advertencia_lines) if advertencia_lines else "-"

    answer = (
        f"Respuesta: {respuesta.strip()}\n\n"
        f"Datos:\n{datos}\n\n"
        f"Fuentes:\n{fuentes}\n\n"
        f"Confianza: {round(confidence, 2)}\n\n"
        f"Advertencias:\n{advertencias}\n"
    )
