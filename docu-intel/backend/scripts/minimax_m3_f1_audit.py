...
# Ensure ``app`` is importable when the script is launched from
# the host (developer checkout) or from the docker container.
_THIS_DIR = Path(__file__).resolve().parent
_BACKEND = _THIS_DIR.parent
if str(_BACKEND) not in sys.path:
    sys.path.insert(0, str(_BACKEND))

from app.services.classification import classify_document  # noqa: E402
from app.services.classification_v2 import classify_multidim  # noqa: E402


MANIFEST_PATH = (
    Path(__file__).parent.parent / "tests" / "fixtures" / "minimax_m3_eval" / "manifest.sanitized.json"
)
if not MANIFEST_PATH.exists():
    # Fall back to the host-side path so the script also works
    # from a developer checkout without docker.
    MANIFEST_PATH = (
        Path(r"C:\Users\Usuario\Desktop\OCR\OCR\docu-intel\backend\tests\fixtures\minimax_m3_eval\manifest.sanitized.json")
    )
CORPUS_PATH = Path(r"D:\TEST2025\2025\BON PLA SOCIEDAD ANONIMA")
if not CORPUS_PATH.exists():
    # In-container the D: drive is bind-mounted under the
    # same path. If it is missing, the script emits a clear
    # error rather than silently returning zero F1.
    CORPUS_PATH = Path("/d/TEST2025/2025/BON PLA SOCIEDAD ANONIMA")
...
def _resolve_real_paths() -> dict[str, str]:
    ...
    mapping by walking the corpus in the same order the manifest
    was built (subdir by subdir, then alphabetical files inside
    each subdir). The walk order is:

        Presupuesto 250398/CORREOS
        Presupuesto 250398/EXCEL
        Presupuesto 250398/IMAGENES
        Presupuesto 250398/PDF
        Presupuesto 250398/WORD
        Presupuesto 252519/CORREOS
        Presupuesto 252519/PDF
    """
    subdirs = [
        "Presupuesto 250398/CORREOS",
        "Presupuesto 250398/EXCEL",
    ...
    real_files: list[str] = []
    for sub in subdirs:
        sub_path = CORPUS_PATH / sub
        if not sub_path.exists():
            continue
        real_files.extend(
            sorted(p.name for p in sub_path.iterdir() if p.is_file())
        )
    with open(MANIFEST_PATH, "r", encoding="utf-8") as fh:
        data = json.load(fh)
    return {
